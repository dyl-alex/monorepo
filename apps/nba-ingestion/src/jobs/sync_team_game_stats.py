...
def _existing_team_stat_keys(season: str, season_type: str) -> set[tuple[str, int]]:
    sql = """
...
def sync_team_game_stats(season: str, season_type: str | None = None, only_missing: bool = False) -> None:
    ...
    existing_keys = _existing_team_stat_keys(season, season_type) if only_missing else set()
    ...
    logger.info(
        "Completed baseline sync_team_game_stats season=%s processed_rows=%s upserted_rows=%s duplicate_rows_skipped=%s existing_rows_skipped=%s",
        season,
        processed_rows,
        total_rows,
